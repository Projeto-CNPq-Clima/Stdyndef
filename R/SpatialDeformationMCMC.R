...
#' @examples
#'
SpatialDeformationMCMC <- function(response, FT, MatFFT, GT, sites, GAMA,
  prior=list(m0=as.matrix(rep(0,nrow(FT))),
  mvarphi=1/18,
  ...
  for(j in 1:iteration){
    ...
    if(j<=burnin){
      ...
      if((j%%jump)==0){
        #source("C:\\Run ApplyDinac\\mainsubrecord.txt")

      }else{
        #source("C:\\Run ApplyDinac\\mainsub.txt")
